...
import Details from "../../Components/ThreeD/Details.jsx";
...
const fetchTickets = useCallback(async () => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/3d/show-tickets`,
      { loginId }
    );
    ...
    data[0]?.tickets.forEach((ticket) => {
      // Format ticketNumbers
      let formattedTicketNo = "";
      if (Array.isArray(ticket.ticketNumbers)) {
        formattedTicketNo = ticket.ticketNumbers
          .map((i) => `${i.number} : ${i.quantity}`)
          .join(", ");
      }
    ...
    console.error(err);
    toast.error("Failed to load 3D tickets.");
  } finally {
    setLoading(false);
  }
}, [loginId]);
...
const fetchCancelledTickets = useCallback(async () => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/3d/cancelled-tickets`,
      { loginId }
    );
    ...
    console.error(err);
    toast.error("Failed to load cancelled 3D tickets.");
  } finally {
    setLoading(false);
  }
}, [loginId]);
...
const handleCancel = async (ticketId) => {
  ...
  const confirmed = window.confirm(
    "Are you sure you want to cancel this 3D ticket?"
  );
  if (!confirmed) return;
  ...
  try {
    ...
    await axios.post(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/3d/cancel-ticket`,
      { ticketId }
    );

    setTickets((prev) => prev.filter((row) => row.id !== ticketId));
    toast.success("3D Ticket cancelled successfully.");
  } catch (err) {
    console.error(err);
    toast.error("Failed to cancel 3D ticket.");
  } finally {
    setLoading(false);
